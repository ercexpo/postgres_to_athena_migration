...
do
  ...
  java -jar ./orc-tools-1.6.7-uber.jar convert "./migration_files/files/web_historian/${current_week}/web_historian_data.json" -o "./migration_files/orc_files/web_historian/${current_week}/web_historian_data.orc" -s 'struct<id:int,source:string,generator:string,created_utc:timestamp,created_str:string,generated_at:string,recorded_utc:timestamp,recorded_str:string,properties:struct<id:string,url:string,title:string,domain:string,visitId:string,transType:string,refVisitId:string,searchTerms:string,properties_date:decimal(19,4),protocol:string,passive_data_metadata:struct<source:string,generator:string,generator_id:string,pdk_timestamp:decimal(19,7),encrypted_transmission:boolean>>,generator_identifier:string,secondary_identifier:string,user_agent:string,server_generated:boolean,generator_definition_id:int,source_reference_id:int>' -t "yyyy-MM-dd HH:mm:ss.n" 2>&1 | tee ./migration_files/orc_output.log && \
